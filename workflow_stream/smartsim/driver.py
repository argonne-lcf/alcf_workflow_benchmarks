...
# CPU bindings
# Colocated: producer and consumer share each node
COLOCATED_BINDINGS = {
    1:  {"producer": "list:1",
         "consumer": "list:53"},
    2:  {"producer": "list:1:8",
         "consumer": "list:53:60"},
    6:  {"producer": "list:1:8:16:24:32:40",
         "consumer": "list:53:60:68:76:84:92"},
}

# Clustered: producer and consumer run on separate nodes
CLUSTERED_BINDINGS = {
    1:  "list:1",
    2:  "list:1:8",
    8:  "list:1:8:16:24:53:60:68:76",
    12: "list:1:8:16:24:32:40:53:60:68:76:84:92",
}

# DB cores for colocated
COLOCATED_DB_BIND = [50, 51, 102, 103]


def get_colocated_binding(ppn, role):
    """Return the cpu-binding string for a colocated (producer|consumer) rank at ppn."""
...
def get_clustered_binding(ppn):
    """Return the cpu-binding string for a clustered producer or consumer rank at ppn."""
...
def launch_coDB(args, nodelist, nNodes):
    ...
    client_exe = os.path.join(os.path.dirname(__file__), 'producer')
    ...
    nrs_settings.set_cpu_binding_type(get_colocated_binding(args.ppn, "producer"))
    nrs_settings.add_exe_args(" ".join(args.producer_args))

    # Create the co-located database model
    colo_model = exp.create_model("producer", nrs_settings)
    kwargs = {
        'maxclients': 100000,
    ...
    colo_model.colocate_db_uds(
            db_cpus=len(COLOCATED_DB_BIND),
            custom_pinning=COLOCATED_DB_BIND,
            debug=False,
            **kwargs
            )

    # Start the co-located model
    print("Launching producer and SmartSim co-located DB ... ")
    exp.generate(colo_model, overwrite=True)
    exp.start(colo_model, block=False, summary=False)
    print("Done\n")

    # Setup and launch the consumer script
    ml_exe = os.path.join(os.path.dirname(__file__), 'consumer.py')
    ...
    ml_settings.set_cpu_binding_type(get_colocated_binding(args.ppn, "consumer"))

    print("Launching consumer ... ")
    ml_model = exp.create_model("consumer", ml_settings)
...
def launch_clDB(args, nodelist, nNodes):
    ...
    if (nodelist is not None):
        ...
        if num_remaining_nodes%2 == 0:
            ...
            print(f"Error: Need even number of nodes, got {remaining_nodes}")
            exit(1)
        print(f"Database running on {args.db_nodes} nodes:")
        print(dbNodes)
        print(f"Producer running on {num_sim_nodes} nodes:")
        print(simNodes)
        print(f"Consumer running on {num_ml_nodes} nodes:")
        print(mlNodes)
    ...
    nrs_settings.set_cpu_binding_type(get_clustered_binding(args.ppn))
    nrs_settings.add_exe_args(" ".join(args.producer_args))

    # Start the client model
    print("Launching the producer ...")
    sim_model = exp.create_model("producer", nrs_settings)
    ...
    ml_settings.set_cpu_binding_type(get_clustered_binding(args.ppn))
...
def main():
    ...
    parser.add_argument("--name", type=str, default="ssim_workflow_stream", help="Name of the experiment")
    ...
    parser.add_argument("--producer_args", type=str, nargs='+', default=[], help="Arguments for producer")
