...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        std::cout << "Running with " << size << " MPI ranks and "
                  ...
                  << "per rank" << std::endl;
        std::cout << "Buffer location (producer and consumer): " << device << "\n" << std::endl;
    }
    ...
    if (on_gpu) {
        ...
        if (gpu_devices.empty()) {
            std::cerr << "[rank " << rank << "] No Level Zero GPU devices found!" << std::endl;
            MPI_Abort(comm, 1);
        }
    ...
    if (rank == 0) {
        ...
        std::cout << "Buffer location (producer and consumer): " << device << std::endl;
